...
variables['a'] = 1-2
variables['b'] = 3 + 5
variables['c'] = variables.get('a',0)+variables.get('b',0)
q = "what is paris"
...
if 'answer' in variables:
    ...
    html_output.append(f'<span style="color:#2ef">{combined}</span><br>')
val = variables.get('a',0)+variables.get('b',0)
html_output.append(f'<span style="color:cyan">{val}</span><br>')

html_result = "\n".join(html_output)
with open(r'E:/Neels Programs/first lang/N# projects/compiled/test2_compiled.html', 'w', encoding='utf-8') as f:
    f.write(html_result)
print("HTML snapshot written to E:/Neels Programs/first lang/N# projects/compiled/test2_compiled.html")
